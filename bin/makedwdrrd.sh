#!/bin/sh
START=$(( $(date -d$(cut -d"," -f2 /opt/klima/graphics/de-dwd-nkdz-req-TAUUHV.csv|sort|head -1) +%s) - 1 ))
...
for t in $(cut -d"," -f2 /opt/klima/graphics/de-dwd-nkdz-req-TAUUHV.csv|sort)
do
  tics=$(date -d "$t" +%s)
